...
import { addUser, deleteUser, findAllUsers, findUserDetail, generateOtp, hashPassword, matchPassword, sendEmail, updateUser, updateUserData } from '../../services/index.js';
...
//Response Messages
const { ALREADY_EXIST, REGISTERD, FETCH_USERS, LOGIN, DELETE_USER, UPDATE_USER, OTP_FOR_PASSWORD, EMAIL_NOT_REGISTER, OTP_MISMATCH, VERIFY_OTP, RESET_PASSWORD, INVALID_PASSWORD, PASSWORD_CHANGED } = responseMessages;

const router = Router();


//Add user
router.post('/sign-up', validators('ADD_USER'), upload.fields([{ name: 'image', maxCount: 1 }]), catchAsyncAction(async (req, res) => {
    ...
    return makeResponse(res, SUCCESS, true, REGISTERD, newUserMapper, { accessToken, refreshToken });
}));
...
//Change Password
router.patch('/change-password', userAuth, validators('CHANGE_PASSWORD'), (req, res) => {
    const { email, password } = req.userData;
    matchPassword(req.body.oldPassword, password)
        .then(async result => {
            if (result) {
                return updateUser(email, { password: await hashPassword(req.body.newPassword) });
            }
...
router.post('/verify-otp', validators('VERIFY_OTP'), catchAsyncAction(async (req, res) => {
    ...
    if (userRecord.otp === req.body.otp) return makeResponse(res, SUCCESS, true, VERIFY_OTP)
...
//Update user details
router.patch('/update-user', upload.fields([{ name: 'image', maxCount: 1 }]), userAuth, catchAsyncAction(async (req, res) => {
